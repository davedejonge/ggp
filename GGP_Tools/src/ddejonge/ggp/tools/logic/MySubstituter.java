...
public final class MySubstituter
{
	...
	{
		if (theta == null || function.isGround())
		{
			return function;
		}
		else
	...
	private static GdlLiteral substituteLiteral(GdlLiteral literal, Substitution theta)
	{
		...
		else if (literal instanceof GdlOr)
		{
			...
			return substituteAnd((GdlAnd)literal, theta);
		}
		else
		{
	...
	{
		if (theta == null || or.isGround())
		{
			return or;
		}
		else
		{
			Disjunction disjuncts = new Disjunction();
	...
	private static GdlAnd substituteAnd(GdlAnd and, Substitution theta)
	{
		if (theta == null || and.isGround())
		{
			return and;
		}
		else
		{
			Conjunction conjuncts = new Conjunction();
			for (int i = 0; i < and.arity(); i++)
			{
				conjuncts.add(substituteLiteral(and.get(i), theta));
			}
	...
	{
		if (theta == null || relation.isGround())
		{
			return relation;
		}
		else
	...
	{
		if (theta == null || !theta.contains(variable))
		{
			return variable;
		}
		else
